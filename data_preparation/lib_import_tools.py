...
def map_protein_id_to_locus_id(IDMappingFile): 
    ...
    This function returns a dict that maps Uniprot ID to gene ordered locus name. 
...
def map_locus_id_to_protein_id(IDMappingFile): 
    '''
    ...
    with open(IDMappingFile, 'r') as f: 
        ...
        for line in lines: 
            elements = line.rstrip().split('\t')
            if elements[1] == 'Gene_OrderedLocusName': 
                IDMappingDict[elements[2]] = elements[0]
    return IDMappingDict

def map_protein_id_to_gene_name(IDMappingFile): 
    '''

    This function returns a dict that maps Uniprot ID to gene name. 
...
def map_gene_name_to_locus_id(IDMappingFile): 
    '''

    This function returns a dict that maps gene name to Uniprot ID. 
    ...
    with open(IDMappingFile, 'r') as f: 
        ...
        for line in lines: 
            ...
            if len(elements) != 3:
                continue
            uniprot_id, alias_type, alias_name = elements
            if uniprot_id not in temp_dict:
                temp_dict[uniprot_id] = {}
            temp_dict[uniprot_id][alias_type] = alias_name
    for uniprot_id, aliases in temp_dict.items():
        if 'Gene_Name' in aliases and 'Gene_OrderedLocusName' in aliases:
            gene_alias_dict[aliases['Gene_Name']] = aliases['Gene_OrderedLocusName']
